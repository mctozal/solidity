...
)
{
	...
	))
	{
		...
		return {{_arguments, SourceLocation{}}};
	}
	...
	if (!sourceIndex.has_value() || !start.has_value() || !end.has_value())
		...
		return {{_arguments, SourceLocation{start.value(), end.value(), nullptr}}};
	...
	else
	{
		...
		return {{_arguments, SourceLocation{start.value(), end.value(), move(sourceName)}}};
	}
	return {{_arguments, SourceLocation{}}};
}
